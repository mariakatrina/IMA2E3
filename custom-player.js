...
// // // class returns an array to the only element in this list
...
// removes reg controls and replaces w custom controls

/* add an event listener for clicking the play/pause button and then define it's functionality */

play.addEventListener('click', playPauseMedia);

// when play button is 'click'ed play or pause media
// allows flexibility for how this function can work

video.addEventListener('click', playPauseMedia);

// applied play/pause function on the video itself  as noticed from
// most video players today 
...
function mediaForward() {
  ...
  if(fwd.classList.contains("active")) {
    fwd.classList.remove("active");
    clearInterval(intervalFwd);
    // var declared globally, can be used in all fn
    media.play();
  } else {
    fwd.classList.add("active");
    media.pause();
    intervalFwd = setInterval(windForward, 200);
    // first attr is a fn below
    // runs fn forward 200ms
    // be mindful of the time, allow computer to allow this loop in set amt of time
...
// playing back audio content to allow some more agency and control for users
volSlider.addEventListener('input', (e) => {
  changeVolume(e.target.value);
});

// abstracted into a new function

function changeVolume(newVolume) {
  media.volume = newVolume;
}

// looks for the click event on the fullscreen element to enable fullscreen
fullscreen.addEventListener("click", openFullScreen);

// this is the function that enables the video to full screen mode
// else if statements enables this function request browser availability
function openFullScreen() {
  if (video.requestFullScreen) {
    video.requestFullScreen();
  } else if (video.webkitRequestFullScreen) {
    video.webkitRequestFullScreen(); // enables fullscreen in Safari
  } else if (video.mozRequestFullScreen) {
    video.mozRequestFullScreen(); // enables fullscreen in mozilla firefox
  } else if (video.msRequestFullscreen) { 
    video.msRequestFullscreen(); // enables fullscreen in Internet Explorer
  }
};


/* functions below are used to experiment with the 'Preview' feature thats available 
in many video players (especially on youtube). I noticed how this would be helpful when
viewing video thumbnails and not the actual video expereince itself because it interrupts
with the video playing (restarting the video everytime the cursor hovers on the video) */
...
//   startPreview();
//   previewTimeout = setTimeout(stopPreview, 50000);
// });
